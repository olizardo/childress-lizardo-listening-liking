...
Because our data is structured with repeated observations across respondents ($N = 1,574$) and genres ($N = 20$), we evaluate three nested multinomial modeling strategies:
...
3. **Genre Fixed Effects + Arts Interactions (`nnet::multinom`)**: Adds genre-specific slopes via `child_arts * genre_id` interaction terms to capture genre-contingent returns to cultural capital.
...
if (file.exists("model_fixed_genre.rds")) {
  m_fixed_genre <- readRDS("model_fixed_genre.rds")
} else {
...
fit_gt <- fit_table %>%
  ...
  tab_header(
    ...
    subtitle = "Comparing Pooled, Genre Fixed-Effects, and Genre Interaction Specifications"
  ) %>%
  cols_label(
  ...
  tab_footnote(
    footnote = "Model 3 (Genre FEs + Arts Interactions) achieves the lowest AIC and residual deviance, confirming substantial genre heterogeneity in the returns to arts exposure.",
...
The fit comparison demonstrates that incorporating genre-specific intercepts and genre $\times$ arts interactions substantially improves model fit, reducing residual deviance by over 6,500 points relative to the pooled model and lowering the AIC from 58,977.5 down to 52,686.0.

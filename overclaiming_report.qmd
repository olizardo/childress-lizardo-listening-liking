...
p_margins <- ggplot(pred_df_rob, aes(x = child_arts, y = estimate, color = response.level)) +
  ...
  scale_y_continuous(labels = scales::percent_format(accuracy = 1), breaks = seq(0, 0.6, by = 0.1), limits = c(0, 0.6)) +
  theme_minimal(base_size = 14) +
  theme(
...
To formally test this, we evaluate three nested models:
1. **Base Demographics**: A pooled model assuming demographic effects (and the baseline rate of overclaiming) are identical across all genres.
2. **Genre Fixed Effects**: Adding independent intercepts for all 20 genres (accounting for the fact that Classical is inherently overclaimed more globally than Country, independent of demographics).
3. **Genre Interactions**: Adding an interaction between childhood arts exposure and genre (allowing the specific *slope* or effect of arts exposure to vary uniquely for every single genre).
...
#| echo: false
# Fit intermediate genre fixed-effects model
model_fixed_genre <- multinom(
...
fit_table <- data.frame(
  Model = c(
    "1. Base Demographics", 
    "2. Base + Genre Fixed Effects", 
    "3. Base + Genre FEs + Arts × Genre Interactions"
  ),
  AIC = c(AIC(model_fixed), AIC(model_fixed_genre), AIC(model_int)),
  BIC = c(BIC(model_fixed), BIC(model_fixed_genre), BIC(model_int)),
  Deviance = c(deviance(model_fixed), deviance(model_fixed_genre), deviance(model_int))
)
...
fit_gt <- fit_table %>%
  ...
  tab_header(
    ...
    subtitle = "Lower AIC and BIC indicate better model fit"
  ) %>%
  fmt_number(columns = c(AIC, BIC, Deviance), decimals = 0) %>%
  opt_stylize(style = 6, color = "cyan")

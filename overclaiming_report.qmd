...
### Overclaiming Across Different Genres (Random Coefficients)

To test whether early arts exposure increases overclaiming uniformly across all genres, or if it specifically drives overclaiming of culturally legitimated (highbrow) genres, we extract the Average Marginal Effects directly from the crossed random-slopes model (`mclogit::mblogit`, random = `list(~ 1|id, ~ 1 + child_arts|genre_id)`). In this mixed model, each genre's slope is shrunk toward the global mean via empirical Bayes shrinkage (BLUPs), providing robust estimates of cross-genre heterogeneity.
...
delta_p_both <- p_plus1[, "Both"] - p_orig[, "Both"]
delta_p_under <- p_plus1[, "ListenOnly"] - p_orig[, "ListenOnly"]
...
# Compute summary stats by genre for Both
ame_both_stats <- tapply(delta_p_both, genre_vec, function(x) {
  c(ame = mean(x), se = sd(x)/sqrt(length(x)), sd = sd(x))
})
ame_both_mat <- do.call(rbind, ame_both_stats)
ame_both_df <- data.frame(genre_id = rownames(ame_both_mat), ame_both_mat, stringsAsFactors = FALSE) %>%
  left_join(genre_labels, by = "genre_id") %>%
  mutate(
    conf.low = ame - 1.96 * se,
    conf.high = ame + 1.96 * se
...
# Compute summary stats by genre for ListenOnly (Underclaim)
ame_under_stats <- tapply(delta_p_under, genre_vec, function(x) {
  c(ame = mean(x), se = sd(x)/sqrt(length(x)), sd = sd(x))
})
ame_under_mat <- do.call(rbind, ame_under_stats)
ame_under_df <- data.frame(genre_id = rownames(ame_under_mat), ame_under_mat, stringsAsFactors = FALSE) %>%
  left_join(genre_labels, by = "genre_id") %>%
  mutate(
    conf.low = ame - 1.96 * se,
    conf.high = ame + 1.96 * se
...
p_ame_rc <- ggplot(ame_mblogit_df, aes(x = ame, y = genre_name)) +
  ...
  labs(
    ...
    subtitle = "Crossed Multilevel Model (mblogit): AME of a 1-unit increase in Childhood Arts Exposure",
...
The random coefficients model yields virtually identical results to the fixed-effects interaction model: early arts exposure selectively drives overclaiming in Opera, Classical, Swing, Musicals, Latin, and Jazz (ranging from +5.6 to +7.7 percentage points), while having a negligible effect on Country, Classic Rock, Contemporary Pop, and Rap. This demonstrates that our findings are completely robust to empirical Bayes shrinkage and hierarchical random-effects estimation.
...
#| fig-width: 9
#| fig-height: 7
p_ame_both <- ggplot(ame_both_df, aes(x = ame, y = genre_name)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray50") +
  ...
  labs(
    title = "Arts Exposure Effect on True Engagement (Random Coefficients Model)",
    subtitle = "Crossed Multilevel Model (mblogit): AME of a 1-unit increase in Childhood Arts Exposure",
...
# Save to Plots directory
ggsave("Plots/ChildArts_AME_by_Genre_Both_RandomCoefficients.png", p_ame_both, width = 9, height = 7, dpi = 300)
...
#| fig-width: 9
#| fig-height: 7
p_ame_under <- ggplot(ame_under_df, aes(x = ame, y = genre_name)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray50") +
  ...
  labs(
    title = "Arts Exposure Effect on Underclaiming (Random Coefficients Model)",
    subtitle = "Crossed Multilevel Model (mblogit): AME of a 1-unit increase in Childhood Arts Exposure",
...
# Save to Plots directory
ggsave("Plots/ChildArts_AME_by_Genre_Underclaim_RandomCoefficients.png", p_ame_under, width = 9, height = 7, dpi = 300)
...
### Overclaiming and Demographic Prestige
...
We can create a "Prestige Index" by averaging two distinct demographic preference ratios, computed directly from the survey respondents' actual abstract liking rates:
...
cor_data <- ame_mblogit_df %>%
  select(genre_id, genre_name, ame, pref_group) %>%
...
spearman_rho <- cor(cor_data$ame, cor_data$avg_ratio, method = "spearman")
cor_label <- sprintf("Spearman \u03c1: %.2f", spearman_rho)

# Plot
p_cor <- ggplot(cor_data, aes(x = avg_ratio, y = ame)) +
  geom_smooth(method = "lm", se = FALSE, color = "gray50", linetype = "dashed") +
  geom_point(aes(color = pref_group), size = 3) +
  geom_text_repel(aes(label = genre_name), size = 3.5, box.padding = 0.5) +
  annotate("text", x = min(cor_data$avg_ratio, na.rm=T), y = max(cor_data$ame, na.rm=T), 
